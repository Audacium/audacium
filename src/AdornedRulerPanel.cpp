...
\brief This is an Audacium Specific ruler panel which additionally
...
public:
   QuickPlayIndicatorOverlay(AudaciumProject *project);
...
public:
   ...
   unsigned DoContextMenu
      (const wxRect &rect,
       wxWindow *pParent, wxPoint *pPosition, AudaciumProject*) final override
   {
...
protected:
   Result Click
      (const TrackPanelMouseEvent &event, AudaciumProject *) override
   {
   ...
   Result Release
      (const TrackPanelMouseEvent &event, AudaciumProject *,
       wxWindow *) override
   {
   ...
   void Enter(bool, AudaciumProject *) override
   {
...
wxCoord GetPlayHeadX( const AudaciumProject *pProject )
{
...
double GetPlayHeadFraction( const AudaciumProject *pProject, wxCoord xx )
{
   const auto &viewInfo = ViewInfo::Get( *pProject );
   auto width = viewInfo.GetTracksUsableWidth();
...
public:
   ...
   HitTest( const AudaciumProject *pProject, wxCoord xx )
   {
...
namespace{
AudaciumProject::AttachedWindows::RegisteredFactory sKey{
[]( AudaciumProject &project ) -> wxWeakRef< wxWindow > {
...
AdornedRulerPanel &AdornedRulerPanel::Get( AudaciumProject &project )
{
   return project.AttachedWindows::Get< AdornedRulerPanel >( sKey );
}

const AdornedRulerPanel &AdornedRulerPanel::Get( const AudaciumProject &project )
{
   return Get( const_cast< AudaciumProject & >( project ) );
}

void AdornedRulerPanel::Destroy( AudaciumProject &project )
{
...
AdornedRulerPanel::AdornedRulerPanel(AudaciumProject* project,
                                     wxWindow *parent,
                                     wxWindowID id,
                                     const wxPoint& pos,
                                     const wxSize& size,
...
(const TrackPanelMouseEvent &event, AudaciumProject *pProject) -> Result
{
   auto result = CommonRulerHandle::Click(event, pProject);
   if (!( result & RefreshCode::Cancelled )) {
      if (mClicked == Button::Left) {
...
(AudaciumProject *pProject) -> Result
{
   auto result = CommonRulerHandle::Cancel(pProject);
...
{
   // This is called by AudaciumProject to make the play region follow
...
AudaciumProject * AdornedRulerPanel::GetProject() const
{
   return mProject;
}
