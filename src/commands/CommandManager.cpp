...
CommandManager &CommandManager::Get( AudaciumProject &project )
{
   return project.AttachedObjects::Get< CommandManager >( key );
}

const CommandManager &CommandManager::Get( const AudaciumProject &project )
{
...
void CommandManager::UpdateCheckmarks( AudaciumProject &project )
{
...
void CommandManager::AddItem(AudaciumProject &project,
...
{
   return [=](AudaciumProject&){ return gPrefs->ReadBool( key, defaultValue ); };
}
...
///
///
bool CommandManager::FilterKeyEvent(AudaciumProject *project, const wxKeyEvent & evt, bool permit)
{
   if (!project)
      return false;
...
bool CommandManager::HandleCommandEntry(AudaciumProject &project,
...
bool CommandManager::HandleMenuID(
   AudaciumProject &project, int id, CommandFlag flags, bool alwaysEnabled)
{
   mLastProcessId = id;
   CommandListEntry *entry = mCommandNumericIDHash[id];
...
TranslatableStrings CommandManager::GetCategories( AudaciumProject& )
{
...
{
   if (!wxStrcmp(tag, wxT("audaciumkeyboard"))) {
      mXMLKeysRead = 0;
   }
...
// may throw
{
   xmlFile.StartTag(wxT("audaciumkeyboard"));
   ...
   xmlFile.EndTag(wxT("audaciumkeyboard"));
}
...
// of these is changed, then if /GUI/Shortcuts/FullDefaults is not set in audacium.cfg,
// because the defaults appear as user assigned shortcuts in audacium.cfg,
...
void CommandManager::RemoveDuplicateShortcuts()
{
   ...
   if (!disabledShortcuts.Translation().empty()) {
      ...
      " that you have assigned to another command.")
         + disabledShortcuts;
      AudaciumMessageBox(message, XO("Shortcuts have been removed"), wxOK | wxCENTRE);
