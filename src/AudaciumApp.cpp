...
\class AudaciumApp
\brief AudaciumApp is the 'main' class for Audacium
...
#include "../images/AudaciumLogoWithName.xpm"
...
void PopulatePreferences()
{
   ...
   if (fn.FileExists())   // it will exist if the (win) installer put it there
   {
      ...
      auto pIni =
         AudaciumFileConfig::Create({}, {}, fullPath, {},
   ...
   if (langCode.empty())
      langCode =
         Languages::GetSystemLanguageCode(FileNames::AudaciumPathList());
...
"Reset Preferences?\n\nThis is a one-time question, after an 'install' where you asked to have the Preferences reset.");
   ...
   // In Audacium 2.1.0 support for the legacy 1.2.x preferences (depreciated since Audacium
   // 1.3.1) is dropped. As a result we can drop the import flag
   // first time this version of Audacium is run we try to migrate
   // old preferences.
   ...
   // Check if some prefs updates need to happen based on audacium version.
   ...
   // (meaning the changes won't mess audacium up if the user goes back to an earlier version)
...
static void QuitAudacium(bool bForce)
{
...
IMPLEMENT_APP_NO_MAIN(AudaciumApp)
...
#else
IMPLEMENT_APP(AudaciumApp)
#endif

#ifdef __WXMAC__

// in response of an open-document apple event
void AudaciumApp::MacOpenFile(const wxString &fileName)
{
   ofqueue.push_back(fileName);
}

// in response of a print-document apple event
void AudaciumApp::MacPrintFile(const wxString &fileName)
{
   ofqueue.push_back(fileName);
}

// in response of a open-application apple event
void AudaciumApp::MacNewFile()
{
   if (!gInited)
      return;
...
#define kAudaciumAppTimerID 0

BEGIN_EVENT_TABLE(AudaciumApp, wxApp)
   EVT_QUERY_END_SESSION(AudaciumApp::OnQueryEndSession)
   EVT_END_SESSION(AudaciumApp::OnEndSession)

   EVT_TIMER(kAudaciumAppTimerID, AudaciumApp::OnTimer)
#ifdef __WXMAC__
   EVT_MENU(wxID_NEW, AudaciumApp::OnMenuNew)
   EVT_MENU(wxID_OPEN, AudaciumApp::OnMenuOpen)
   EVT_MENU(wxID_ABOUT, AudaciumApp::OnMenuAbout)
   EVT_MENU(wxID_PREFERENCES, AudaciumApp::OnMenuPreferences)
#endif
   ...
   // event
   EVT_MENU(wxID_EXIT, AudaciumApp::OnMenuExit)

#ifndef __WXMSW__
   EVT_SOCKET(ID_IPC_SERVER, AudaciumApp::OnServerEvent)
   EVT_SOCKET(ID_IPC_SOCKET, AudaciumApp::OnSocketEvent)
#endif

   // Recent file event handlers.
   EVT_MENU(FileHistory::ID_RECENT_CLEAR, AudaciumApp::OnMRUClear)
   EVT_MENU_RANGE(FileHistory::ID_RECENT_FIRST, FileHistory::ID_RECENT_LAST,
      AudaciumApp::OnMRUFile)

   // Handle AppCommandEvents (usually from a script)
   EVT_APP_COMMAND(wxID_ANY, AudaciumApp::OnReceiveCommand)

   // Global ESC key handling
   EVT_KEY_DOWN(AudaciumApp::OnKeyDown)
END_EVENT_TABLE()
...
bool AudaciumApp::MRUOpen(const FilePath &fullPathStr) {
   // Most of the checks below are copied from ProjectManager::OpenFiles.
   // - some rationalisation might be possible.

   AudaciumProject *proj = GetActiveProject();
   ...
   if (!fullPathStr.empty())
   {
      ...
      if (wxFile::Exists(fullPathStr))
      {
         ...
         // Test here even though AudaciumProject::OpenFile() also now checks, because
...
bool AudaciumApp::SafeMRUOpen(const wxString &fullPathStr)
{
   return GuardedCall< bool >( [&]{ return MRUOpen( fullPathStr ); } );
}

void AudaciumApp::OnMRUClear(wxCommandEvent& WXUNUSED(event))
{
...
void AudaciumApp::OnTimer(wxTimerEvent& WXUNUSED(event))
{
   // Filenames are queued when Audacium receives a few of the
...
void AudaciumApp::OnFatalException()
{
   exit(-1);
}
...
bool AudaciumApp::OnExceptionInMainLoop()
{
...
bool AudaciumApp::OnInit()
{
   ...
   gtk_rc_parse_string("style \"audacium\" {\n"
                       " GtkButton::inner_border = { 0, 0, 0, 0 }\n"
                       " GtkEntry::inner_border = { 0, 0, 0, 0 }\n"
                       " xthickness = 4\n"
                       " ythickness = 0\n"
                       "}\n"
                       "widget_class \"*GtkCombo*\" style \"audacium\"");
#endif

   // Don't use AUDACITY_NAME here.
   // We want Audacium with a capital 'A'
   ...
   /* Search path (for plug-ins, translations etc) is (in this order):
      ...
      * The user's "~/.audacium-data" or "Portable Settings" directory
      * The user's "~/.audacium-files" directory
   ...
   if (!envTempDir.empty()) {
      ...
      TempDirectory::SetDefaultTempDir( wxString::Format(
         wxT("%s/audacium-%s"), envTempDir, wxGetUserId() ) );
   } else {
      /* On Unix systems, the default temp dir is in /var/tmp. */
      TempDirectory::SetDefaultTempDir( wxString::Format(
         wxT("/var/tmp/audacium-%s"), wxGetUserId() ) );
   }
...
// DA: Path env variable.
   ...
   if (!pathVar.empty())
      FileNames::AddMultiPathsToPathList(pathVar, audaciumPathList);
   FileNames::AddUniquePathToPathList(::wxGetCwd(), audaciumPathList);

   wxString progPath = wxPathOnly(argv[0]);
   FileNames::AddUniquePathToPathList(progPath, audaciumPathList);
   // Add the path to modules:
   FileNames::AddUniquePathToPathList(progPath + L"/lib/audacium", audaciumPathList);

   FileNames::AddUniquePathToPathList(FileNames::DataDir(), audaciumPathList);
...
// JKC Bug 1220: Use path based on home directory on WXMAC
...
#ifdef __WXMSW__
   // On Windows, the path to the Audacium program is in argv[0]
   wxString progPath = wxPathOnly(argv[0]);
   FileNames::AddUniquePathToPathList(progPath, audaciumPathList);
   FileNames::AddUniquePathToPathList(progPath + wxT("\\Languages"), audaciumPathList);
...
#ifdef __WXMAC__
   // On Mac OS X, the path to the Audacium program is in argv[0]
   wxString progPath = wxPathOnly(argv[0]);

   FileNames::AddUniquePathToPathList(progPath, audaciumPathList);
   // If Audacium is a "bundle" package, then the root directory is
   // the great-great-grandparent of the directory containing the executable.
   //FileNames::AddUniquePathToPathList(progPath + wxT("/../../../"), audaciumPathList);

   // These allow for searching the "bundle"
   FileNames::AddUniquePathToPathList(
      progPath + wxT("/../"), audaciumPathList);
   FileNames::AddUniquePathToPathList(
      progPath + wxT("/../Resources"), audaciumPathList);
   ...
   TempDirectory::SetDefaultTempDir( wxString::Format(
      wxT("%s/Library/Application Support/audacium/SessionData"), tmpDirLoc) );
   ...
   FileNames::SetAudaciumPathList( std::move( audaciumPathList ) );
   ...
   {
      wxFileName configFileName(FileNames::DataDir(), wxT("audacium.cfg"));
      auto appName = wxTheApp->GetAppName();
      InitPreferences( AudaciumFileConfig::Create(
   ...
   // Audacium is running, we must return from OnInit() to wxWidgets before
...
bool AudaciumApp::InitPart2()
{
   ...
   wxImage logoimage((const char **)AudaciumLogoWithName_xpm);
   ...
   AudaciumProject *project;
   {
      ...
      // as where Audacium project will appear.
      wxRect wndRect;
   ...
   // Workaround Bug 1377 - Crash after Audacium starts and low disk space warning appears
...
/*#if defined(HAVE_UPDATES_CHECK)
   ...
   mTimer.SetOwner(this, kAudaciumAppTimerID);
   mTimer.Start(200);
...
#if defined(__WXMAC__)
   // The first time this version of Audacium is run or when the preferences
   // are reset, execute the "tccutil" command to reset the microphone permissions
   // currently assigned to Audacium.  The end result is that the user will be
   // prompted to approve/deny Audacium access (again).
   //
   // This should resolve confusion of why Audacium appears to record, but only
   // gets silence due to Audacium being denied microphone access previously.
   bool permsReset = false;
   gPrefs->Read(wxT("/MicrophonePermissionsReset"), &permsReset, false);
   if (!permsReset) {
      system("tccutil reset Microphone org.audaciumteam.audacium");
...
void AudaciumApp::InitCommandHandler()
{
...
void AudaciumApp::OnReceiveCommand(AppCommandEvent &event)
{
   wxASSERT(NULL != mCmdHandler);
   mCmdHandler->OnReceiveCommand(event);
}

void AudaciumApp::OnKeyDown(wxKeyEvent &event)
{
...
"Audacium could not find a safe place to store temporary files.\nAudacium needs a place where automatic cleanup programs won't delete the temporary files.\nPlease enter an appropriate directory in the preferences dialog."));
      } else {
         AudaciumMessageBox(XO(
...
// Return true if there are no other instances of Audacium running,
// false otherwise.

bool AudaciumApp::CreateSingleInstanceChecker(const wxString &dir)
{
   wxString name = wxString::Format(wxT("audacium-lock-%s"), wxGetUserId());
   mChecker.reset();
...
"Audacium was not able to lock the temporary files directory.\nThis folder may be in use by another copy of Audacium.\n")
      ...
      // to an already active Audacium.  If successful, we send
      // the first command line argument (the audio file name)
      // to that Audacium for processing.
      wxClient client;
      ...
      for (int i = 0; i < 50; i++)
      {
         ...
         if (conn)
         {
            ...
            {
               // Send an empty string to force existing Audacium to front
      ...
      // There is another copy of Audacium running.  Force quit.
...
bool AudaciumApp::CreateSingleInstanceChecker(const wxString &dir)
{
   ...
   // time Audacium has been run during this boot of the system. In this
   ...
   // If we get here, then Audacium is currently active. So, we connect
   ...
   // Attempt to connect to an active Audacium.
   ...
   // Display Audacium's version if requested
   if (parser->Found(wxT("v")))
   {
      wxPrintf("Audacium v%s\n", AUDACITY_VERSION_STRING);
   ...
   // go to AudaciumApp::MacOpenFile. Forward the file names to the prior
...
void AudaciumApp::OnServerEvent(wxSocketEvent & /* evt */)
{
   wxSocketBase *sock;
...
void AudaciumApp::OnSocketEvent(wxSocketEvent & evt)
{
...
std::unique_ptr<wxCmdLineParser> AudaciumApp::ParseCommandLine()
{
   ...
   /*i18n-hint: This controls the number of bytes that Audacium will
   ...
   /*i18n-hint: This runs a set of automatic tests on Audacium itself */
   parser->AddSwitch(wxT("t"), wxT("test"), _("run self diagnostics"));

   /*i18n-hint: This displays the Audacium version */
   parser->AddSwitch(wxT("v"), wxT("version"), _("display Audacium version"));

   /*i18n-hint: This is a list of one or more files that Audacium
...
void AudaciumApp::OnQueryEndSession(wxCloseEvent & event)
{
   bool mustVeto = false;
...
void AudaciumApp::OnEndSession(wxCloseEvent & event)
{
...
#ifdef HAS_NETWORKING
   audacium::network_manager::NetworkManager::GetInstance().Terminate();
#endif
...
void AudaciumApp::OnMenuAbout(wxCommandEvent & /*event*/)
{
...
void AudaciumApp::OnMenuNew(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
...
void AudaciumApp::OnMenuOpen(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
   // when no project windows are open. This check assures that
...
void AudaciumApp::OnMenuPreferences(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
   // when no project windows are open. This check assures that
...
void AudaciumApp::OnMenuExit(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
   // when no project windows are open. This check assures that
...
//BG: On Windows, associate the aup file type with Audacium
...
*/
#if defined(__WXMSW__) && !defined(__WXUNIVERSAL__) && !defined(__CYGWIN__)
void AudaciumApp::AssociateFileTypes()
{
   ...
   auto DefineType = [&](const wxString &type)
   {
      ...
      if (!root_key.empty())
      {
         associateFileTypes = wxT("Audacium.Project"); // Finally set value for the key
      }
   ...
   if (IsDefined(wxT(".aup3")) && IsDefined(wxT(".aup")) && IsDefined(wxT("Audacium.Project")))
   {
...
"Audacium project (.aup3) files are not currently \nassociated with Audacium. \n\nAssociate them, so they open on double-click?"),
   ...
   else
   {
      ...
      associateFileTypes = wxT("Audacium.Project"); // Finally set value for .AUP key
      associateFileTypes.SetName(root_key + wxT("Audacium.Project"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
         associateFileTypes = wxT("Audacium Project File");
      }

      associateFileTypes.SetName(root_key + wxT("Audacium.Project\\shell"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("Audacium.Project\\shell\\open"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      }

      associateFileTypes.SetName(root_key + wxT("Audacium.Project\\shell\\open\\command"));
      ...
      if (!associateFileTypes.Exists() ||
            (tmpRegAudPath.Find(wxT("audacium.exe")) >= 0))
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("Audacium.Project\\shell\\open\\ddeexec"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("Audacium.Project\\shell\\open\\ddeexec\\Application"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("Audacium.Project\\shell\\open\\ddeexec\\Topic"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
