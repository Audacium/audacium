...
static const TranslatableString
   /* i18n-hint: These are strings for the status bar, and indicate whether Audacium
...
// compatibility
ControlToolBar::ControlToolBar( AudaciumProject &project )
...
ControlToolBar *ControlToolBar::Find( AudaciumProject &project )
{
...
ControlToolBar &ControlToolBar::Get( AudaciumProject &project )
{
   auto &toolManager = ToolManager::Get( project );
   return *static_cast<ControlToolBar*>(
      toolManager.GetToolBar(TransportBarID) );
...
const ControlToolBar &ControlToolBar::Get( const AudaciumProject &project )
{
...
registeredStatusWidthFunction{
   []( const AudaciumProject &, StatusBarField field )
