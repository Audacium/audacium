...
\brief Utility functions for working with our wxConf (gPrefs)
  ...
  Audacium uses wxWidgets' wxConfig class to handle preferences.
...
\verbatim
  ...
  Preference field specification:
   /
      Version					- Audacium Version that created these prefs
...
void EnumSettingBase::Migrate( wxString &value )
{
   ...
   if ( !mOldKey.empty() &&
      ...
      // Audacium.  But further changes will be stored only to the NEW key
