...
bool SpectrumPrefs::Validate()
{
   ...
   if (!mMaxFreq->GetValue().ToLong(&maxFreq)) {
      AudaciumMessageBox( XO("The maximum frequency must be an integer") );
      return false;
   }

   long minFreq;
   if (!mMinFreq->GetValue().ToLong(&minFreq)) {
      AudaciumMessageBox( XO("The minimum frequency must be an integer") );
      return false;
   }

   long gain;
   if (!mGain->GetValue().ToLong(&gain)) {
      AudaciumMessageBox( XO("The gain must be an integer") );
      return false;
   }

   long range;
   if (!mRange->GetValue().ToLong(&range)) {
      AudaciumMessageBox( XO("The range must be a positive integer") );
      return false;
   }

   long frequencygain;
   if (!mFrequencyGain->GetValue().ToLong(&frequencygain)) {
      AudaciumMessageBox( XO("The frequency gain must be an integer") );
      return false;
   }
   ...
   if (!mFindNotesMinA->GetValue().ToLong(&findNotesMinA)) {
      AudaciumMessageBox( XO("The minimum amplitude (dB) must be an integer") );
      return false;
   }

   long findNotesN;
   if (!mFindNotesN->GetValue().ToLong(&findNotesN)) {
      AudaciumMessageBox( XO("The maximum number of notes must be an integer") );
      return false;
   }
...
{
   return [=](wxWindow *parent, wxWindowID winid, AudaciumProject *pProject)
   {
