...
// This isn't really used yet by wxWidgets as patched by Audacium for
// Mac accessibility, as of Audacium 2.3.2, but here it is anyway, keeping the
...
:  wxDialogWrapper(parent, wxID_ANY, XO("Audacium Preferences"),
            wxDefaultPosition,
            wxDefaultSize,
...
void PrefsDialog::OnOK(wxCommandEvent & WXUNUSED(event))
{
   ...
   if (gAudioIO) {
      ...
      // AudaciumProject::GetUpdateFlags().
   ...
   //      so AudaciumProject::UpdatePrefs() or any of the routines it calls must
...
void DoReloadPreferences( AudaciumProject &project )
{
