...
dynamically linked libraries that modify Audacium.  They are plug-ins 
...
/* i18n-hint: Modules are optional extensions to Audacium that add NEW features.*/
...
void ModulePrefs::GetAllModuleStatuses(){
   ...
   // TODO: On an Audacium upgrade we should (?) actually untick modules.
...
void ModulePrefs::PopulateOrExchange(ShuttleGui & S)
{
   ...
   {
      S.AddFixedText(XO(
"These are experimental modules. Enable them only if you've read the Audacium Manual\nand know what you are doing.") );
      S.AddFixedText(XO(
/* i18n-hint preserve the leading spaces */
"  'Ask' means Audacium will ask if you want to load the module each time it starts.") );
      S.AddFixedText(XO(
/* i18n-hint preserve the leading spaces */
"  'Failed' means Audacium thinks the module is broken and won't run it.") );
      S.AddFixedText(XO(
/* i18n-hint preserve the leading spaces */
"  'New' means no choice has been made yet.") );
      S.AddFixedText(XO(
"Changes to these settings only take effect when Audacium starts up."));
      {
