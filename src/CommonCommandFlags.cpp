...
// Weaker predicate.
bool AnyTracksSelectedPred( const AudaciumProject &project )
{
   auto range = TrackList::Get( project ).Selected();
   return !range.empty();
};
...
#ifdef __WXMAC__
      #define CONTROL_KEY "Cmd"
#else
      #define CONTROL_KEY "Ctrl"
#endif

      // i18n-hint: %s will be replaced by the name of an action, such as Normalize, Cut, Fade.
      format = XO("Select the audio for %s to use (for example, %s + A to Select All) then try again."
          // No need to explain what a help button is for.
          // "\n\nClick the Help button to learn more about selection methods."
      );

      return format.Format( Name, CONTROL_KEY );
   },
...
const ReservedCommandFlag&
   NoAutoSelect() { static ReservedCommandFlag flag{
     [](const AudaciumProject &){ return true; }
   }; return flag; } // jkc
;
