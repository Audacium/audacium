...
AudaciumFileConfig::AudaciumFileConfig(
...
AudaciumFileConfig::~AudaciumFileConfig() = default;

std::unique_ptr<AudaciumFileConfig> AudaciumFileConfig::Create(
   const wxString& appName,
   const wxString& vendorName,
   const wxString& localFilename,
   const wxString& globalFilename,
...
)
{
   ...
   auto result = std::unique_ptr<AudaciumFileConfig>{
      safenew AudaciumFileConfig{
         appName, vendorName, localFilename, globalFilename, style, conv } };
   result->Init();
   return result;
}

void AudaciumFileConfig::Warn()
{
   wxDialogWrapper dlg(nullptr, wxID_ANY, XO("Audacium Configuration Error"));
   ...
   S.StartVerticalLay(wxEXPAND, 1);
   {
      ...
      S.StartHorizontalLay(wxALIGN_RIGHT, 0);
      {
         S.AddFixedText(
            XO("The following configuration file could not be accessed:\n\n"
               ...
               "If you choose to \"Quit Audacium\", your project may be left in an unsaved "
      ...
      S.StartHorizontalLay(wxALIGN_RIGHT, 0);
      {
         ...
         b = S.Id(wxID_CANCEL).AddButton(XXO("&Quit Audacium"));
   ...
   switch (dlg.ShowModal())
   {
      case wxID_HELP:
         ...
         OpenInDefaultBrowser("https://" +
                              ...
                              "Error:_Audacium_settings_file_unwritable");
      break;
