/*!********************************************************************
  ...
  @file AudaciumException.cpp
  @brief Implements AudaciumException and related
...
AudaciumException::~AudaciumException()
{
}
   ...
   if (!moved) {
      ...
      if ( wxAtomicDec( sOutstandingMessages ) == 0 ) {
         if (exceptionType == ExceptionType::Internal)
         {
            ShowExceptionDialog(
               nullptr,
               (caption.empty() ? AudaciumMessageBoxCaptionStr() : caption),
