...
const int AudaciumProjectTimerID = 5200;

static AudaciumProject::AttachedObjects::RegisteredFactory sProjectManagerKey {
   []( AudaciumProject &project ) {
      return std::make_shared< ProjectManager >( project );
   }
};

ProjectManager &ProjectManager::Get( AudaciumProject &project )
{
   return project.AttachedObjects::Get< ProjectManager >( sProjectManagerKey );
}

const ProjectManager &ProjectManager::Get( const AudaciumProject &project )
{
   return Get( const_cast< AudaciumProject & >( project ) );
}

ProjectManager::ProjectManager( AudaciumProject &project )
   : mProject{ project }
   , mTimer{ std::make_unique<wxTimer>(this, AudaciumProjectTimerID) }
{
...
// PRL:  This event type definition used to be in AudaciumApp.h, which created
...
BEGIN_EVENT_TABLE( ProjectManager, wxEvtHandler )
   ...
   EVT_TIMER(AudaciumProjectTimerID, ProjectManager::OnTimer)
END_EVENT_TABLE()
...
void ProjectManager::SaveWindowSize()
{
   ...
   else
   {
      if (foundIconizedProject) {
         ...
         // Audacium would have to be running with no projects open
...
{
public:
   DropTarget(AudaciumProject *proj)
   {
      mProject = proj;
...
private:
   ...
   extern void AddPages(   AudaciumProject * pProj, GuiFactory & Factory,  wxNotebook  * pNotebook );
#endif
...
AudaciumProject *ProjectManager::New()
{
   wxRect wndRect;
   ...
   auto sp = std::make_shared< AudaciumProject >();
   AllProjects{}.Add( sp );
   auto p = sp.get();
   auto &project = *p;
   ...
   //   SetDropTarget(safenew AudaciumDropTarget(this));
   //   mTrackPanel->SetDropTarget(safenew AudaciumDropTarget(this));
...
void ProjectManager::OnCloseWindow(wxCloseEvent & event)
{
   ...
   // AudaciumProject::~AudaciumProject() and TrackPanel::OnTimer().
   ...
   // before the TrackPanel is destroyed. This change was needed to stop Audacium
...
void ProjectManager::OpenFiles(AudaciumProject *proj)
{
   auto selectedFiles =
...
bool ProjectManager::SafeToOpenProjectInto(AudaciumProject &proj)
{
...
AudaciumProject *ProjectManager::OpenProject(
   AudaciumProject *pGivenProject, const FilePath &fileNameArg,
