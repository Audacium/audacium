...
bool AUPImportFileHandle::Open()
{
   ...
   if (ff.IsOpened())
   {
      ...
      if (!wxStrncmp(buf, wxT("AudaciumProject"), 15))
      {
         AudaciumMessageBox(
            XO("This project was saved by Audacium version 1.0 or earlier. The format has\n"
               "changed and this version of Audacium is unable to import the project.\n\n"
               "Use a version of Audacium prior to v3.0.0 to upgrade the project and then\n"
               "you may import it with this version of Audacium."),
            XO("Import Project"),
            wxOK | wxCENTRE,
            &GetProjectFrame(mProject));
...
bool AUPImportFileHandle::HandleXMLTag(const wxChar *tag, const wxChar **attrs)
{
   ...
   if (mCurrentTag.IsSameAs(wxT("project")) ||
       mCurrentTag.IsSameAs(wxT("audaciumproject")))
   {
...
#else
   AudaciumMessageBox(
      XO("MIDI tracks found in project file, but this build of Audacium does not include MIDI support, bypassing track."),
      XO("Project Import"),
...
bool AUPImportFileHandle::HandleWaveTrack(XMLTagHandler *&handler)
{
   ...
   // No active clip.  In early versions of Audacium, there was a single
...
bool AUPImportFileHandle::HandleEnvelope(XMLTagHandler *&handler)
{
   ...
   // Earlier versions of Audacium had a single implied waveclip, so for
