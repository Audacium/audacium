...
#include "../Audacium.h"	// needed before GStreamer.h // for USE_* macros
...
// Convenience macros
#define AUDCTX "audacium::context"
...
// Capabilities that Audacium can handle
//
...
private:
   ...
   Tags                    mTags;         //!< Tags to be passed back to Audacium
...
/// the Audacium import plugin list
...
public:
   ...
   std::unique_ptr<ImportFileHandle> Open(
      const wxString &Filename, AudaciumProject*) override;
};
...
Importer::RegisteredImportPlugin{ "GStreamer",
   ...
   wxLogMessage(_TS("Audacium is built against GStreamer version %d.%d.%d-%d"),
                GST_VERSION_MAJOR,
                GST_VERSION_MINOR,
                GST_VERSION_MICRO,
                GST_VERSION_NANO);
...
GStreamerImportPlugin::GetSupportedExtensions()
{
   ...
   // user had installed additional gstreamer plugins while Audacium
   // was active.
   mExtensions.clear();
...
std::unique_ptr<ImportFileHandle> GStreamerImportPlugin::Open(
   const wxString &filename, AudaciumProject*)
{
