...
namespace {
   void Notify( AudaciumProject &project, ProjectSettings::EventCode code )
   {
...
static const AudaciumProject::AttachedObjects::RegisteredFactory
sProjectSettingsKey{
...
ProjectSettings &ProjectSettings::Get( AudaciumProject &project )
{
   return project.AttachedObjects::Get< ProjectSettings >(
      sProjectSettingsKey );
}

const ProjectSettings &ProjectSettings::Get( const AudaciumProject &project )
{
   return Get( const_cast< AudaciumProject & >( project ) );
}

ProjectSettings::ProjectSettings(AudaciumProject &project)
   : mProject{ project }
...
, mSnapTo( gPrefs->Read(wxT("/SnapTo"), SNAP_OFF) )
{
   ...
   if ( !wasDefined ) {
      ...
      // an entry for the default sample rate in audacium.cfg, Audacium can open
