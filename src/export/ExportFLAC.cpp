...
private:
   ...
   bool GetMetadata(AudaciumProject *project, const Tags *tags);
...
ProgressResult ExportFLAC::Export(AudaciumProject *project,
                        std::unique_ptr<ProgressDialog> &pDialog,
                        ...
                        int WXUNUSED(subformat))
{
   ...
   if (!f.Open(path, wxT("w+b"))) {
      AudaciumMessageBox( XO("FLAC export couldn't open %s").Format( path ) );
      return ProgressResult::Cancelled;
   }
...
bool ExportFLAC::GetMetadata(AudaciumProject *project, const Tags *tags)
{
