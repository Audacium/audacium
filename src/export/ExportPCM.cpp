...
private:
   ...
   bool AddStrings(AudaciumProject *project, SNDFILE *sf, const Tags *tags, int sf_format);
   bool AddID3Chunk(
...
void ExportPCM::ReportTooBigError(wxWindow * pParent)
{
   ...
   auto message =
      ...
      "Audacium cannot do this, the Export was abandoned.");
...
// compute the size in advance.
   ...
   ShowErrorDialog(pParent, XO("Error Exporting"),
                  XO("Your exported WAV file has been truncated as Audacium cannot export WAV\n"
 ...
 */
ProgressResult ExportPCM::Export(AudaciumProject *project,
                                 std::unique_ptr<ProgressDialog> &pDialog,
                                 ...
                                 int subformat)
{
   ...
   auto updateResult = ProgressResult::Success;
   {
      ...
      {
         AudaciumMessageBox( XO("GSM 6.10 requires mono") );
      ...
      if (!sf_format_check(&info)) {
         AudaciumMessageBox( XO("Cannot export audio in this format.") );
         return ProgressResult::Cancelled;
      }
      ...
      if (!sf) {
         AudaciumMessageBox( XO("Cannot export audio to %s").Format( path ) );
         return ProgressResult::Cancelled;
      }
...
bool ExportPCM::AddStrings(AudaciumProject * WXUNUSED(project), SNDFILE *sf, const Tags *tags, int sf_format)
{
