...
public:
   ...
   bool Init(const char *shortname, AudaciumProject *project, const Tags *metadata, int subformat);

   /// Codec initialization
   bool InitCodecs(AudaciumProject *project);
   ...
   /// Exports audio
   ///\param project Audacium project
...
bool ExportFFmpeg::Init(const char *shortname, AudaciumProject *project, const Tags *metadata, int subformat)
{
...
"FFmpeg : ERROR - Can't determine format description for file \"%s\".")
   ...
   // At the moment Audacium can export only one audio stream
...
bool ExportFFmpeg::InitCodecs(AudaciumProject *project)
{
...
//FIXME The list of supported options for the selected encoder should be extracted instead of a few hardcoded
   ...
   if (mEncAudioCodecCtx->codec_id == AV_CODEC_ID_AC3)
   {
      ...
      // But, currently, Audacium only supports SAMPLE_FMT_S16.  So, for now, look for the
...
ProgressResult ExportFFmpeg::Export(AudaciumProject *project,
   std::unique_ptr<ProgressDialog> &pDialog,
