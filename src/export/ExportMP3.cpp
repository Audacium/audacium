...
public:
   ...
   void PopulateOrExchange(ShuttleGui & S)
   {
      ...
      {
         S.AddTitle(
            XO("Audacium needs the file %s to create MP3s.")
               .Format( mName ) );
...
// Debug/Release builds of Audacium and the lame DLL.
...
bool MP3Exporter::LoadLibrary(wxWindow *parent, AskUser askuser)
{
   ...
   if (!ValidLibraryLoaded()) {
      ...
      if (askuser && !mBladeVersion.empty()) {
         AudaciumMessageBox( mBladeVersion );
      }
#endif
...
bool MP3Exporter::InitLibraryExternal(wxString libpath)
{
      ...
      !lame_set_bWriteVbrTag)
   {
      ...
      if (beVersion) {
         ...
         mBladeVersion = XO(
"You are linking to lame_enc.dll v%d.%d. This version is not compatible with Audacium %d.%d.%d.\nPlease download the latest version of 'LAME for Audacium'.")
            .Format(
               v.byMajorVersion,
               v.byMinorVersion,
               AUDACITY_VERSION,
...
{
   wxRegKey reg(wxT("HKEY_LOCAL_MACHINE\\Software\\Lame for Audacium"));
   wxString path;
...
wxString MP3Exporter::GetLibraryPath()
{
   ...
   path = wxT("/Library/Application Support/audacium/libs");
   if (wxFileExists(path + wxT("/") + GetLibraryName()))
   {
        return path;
   }

   path = wxT("/usr/local/lib/audacium");
   if (wxFileExists(path + wxT("/") + GetLibraryName()))
   {
        return path;
   }
...
public:
   ...
   ProgressResult Export(AudaciumProject *project,
...
private:
   ...
   unsigned long AddTags(AudaciumProject *project, ArrayOf<char> &buffer, bool *endOfFile, const Tags *tags);
#ifdef USE_LIBID3TAG
...
bool ExportMP3::CheckFileName(wxFileName & WXUNUSED(filename), int WXUNUSED(format))
{
   ...
   if (!exporter.LoadLibrary(wxTheApp->GetTopWindow(), MP3Exporter::Maybe)) {
      AudaciumMessageBox( XO("Could not open MP3 encoding library!") );
...
ProgressResult ExportMP3::Export(AudaciumProject *project,
                       std::unique_ptr<ProgressDialog> &pDialog,
                       unsigned channels,
                       const wxFileNameWrapper &fName,
                       bool selectionOnly,
                       ...
                       int WXUNUSED(subformat))
{
   ...
   if (!exporter.InitLibrary(wxT(""))) {
      AudaciumMessageBox( XO("Could not initialize MP3 encoding library!") );
      gPrefs->Write(wxT("/MP3/MP3LibPath"), wxString(wxT("")));
      gPrefs->Flush();
   ...
   if (!exporter.ValidLibraryLoaded()) {
      AudaciumMessageBox( XO("Not a valid or supported MP3 encoding library!") );
      gPrefs->Write(wxT("/MP3/MP3LibPath"), wxString(wxT("")));
      gPrefs->Flush();
   ...
   if (((int)inSamples) < 0) {
      AudaciumMessageBox( XO("Unable to initialize MP3 stream") );
      return ProgressResult::Cancelled;
   }
   ...
   if (!outFile.IsOpened()) {
      AudaciumMessageBox( XO("Unable to open target file for writing") );
      return ProgressResult::Cancelled;
   }
...
unsigned long ExportMP3::AddTags(AudaciumProject *WXUNUSED(project), ArrayOf<char> &buffer, bool *endOfFile, const Tags *tags)
{
#ifdef USE_LIBID3TAG
