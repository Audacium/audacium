...
bool FFmpegPresets::SavePreset(ExportFFmpegOptions *parent, wxString &name)
{
...

   {
      ...
      {
         AudaciumMessageBox( XO("Please select format before saving a profile") );
         return false;
      }
      ...
      if (lb->GetSelection() < 0)
      {
         ...
         AudaciumMessageBox( XO("Please select codec before saving a profile") );
         return false;
      }
...
void FFmpegPresets::LoadPreset(ExportFFmpegOptions *parent, wxString &name)
{
   ...
   if (!preset)
   {
      AudaciumMessageBox( XO("Preset '%s' does not exist." ).Format(name));
      return;
   }
...
// may throw
{
   ...
   wxString dtdName = wxT("-//audaciumffmpegpreset-1.0.0//DTD//EN");
   wxString dtdURI =
      wxT("http://audacium.sourceforge.net/xml/audaciumffmpegpreset-1.0.0.dtd");

   xmlFile.Write(wxT("<!DOCTYPE "));
   xmlFile.Write(wxT("project "));
   xmlFile.Write(wxT("PUBLIC "));
   xmlFile.Write(wxT("\"-//audaciumffmpegpreset-1.0.0//DTD//EN\" "));
   xmlFile.Write(wxT("\"http://audacium.sourceforge.net/xml/audaciumffmpegpreset-1.0.0.dtd\" "));
   xmlFile.Write(wxT(">\n"));
}
...
void ExportFFmpegOptions::OnDeletePreset(wxCommandEvent& WXUNUSED(event))
{
   ...
   if (presetname.empty())
   {
      AudaciumMessageBox( XO("You can't delete a preset without name") );
      return;
   }
...
bool ExportFFmpegOptions::SavePreset(bool bCheckForOverwrite)
{
   ...
   if (name.empty())
   {
      AudaciumMessageBox( XO("You can't save a preset without a name") );
      return false;
   }
...
void ExportFFmpegOptions::OnExportPresets(wxCommandEvent& WXUNUSED(event))
{
   ...
   if( presets.Count() < 1)
   {
      AudaciumMessageBox( XO("No presets to export") );
      return;
   }
