...
(AudaciumProject *pProject, float newValue)
{
...
(const wxMouseEvent &, AudaciumProject *pProject)
{
...
TranslatableString GainSliderHandle::Tip(
   const wxMouseState &, AudaciumProject &project) const
{
 ...
 const std::shared_ptr<Track> &pTrack)
{
   ...
   if (sliderRect.Contains(state.m_x, state.m_y)) {
      ...
      auto sliderFn =
      []( AudaciumProject *pProject, const wxRect &sliderRect, Track *pTrack ) {
...
UIHandle::Result PanSliderHandle::SetValue(AudaciumProject *pProject, float newValue)
{
   (void)pProject;//Compiler food
