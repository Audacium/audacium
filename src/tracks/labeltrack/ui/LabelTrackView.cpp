...
(const TrackPanelMouseState &st,
 const AudaciumProject *WXUNUSED(pProject), int, bool)
{
   UIHandlePtr result;
...
bool LabelTrackView::CalcCursorX( AudaciumProject &project, int * x) const
{
...
bool LabelTrackView::IsTextSelected( AudaciumProject &project ) const
{
   if ( !HasSelection( project ) )
      return false;
...
bool LabelTrackView::CutSelectedText( AudaciumProject &project )
{
...
///  @return true if text is selected in text box, false otherwise
bool LabelTrackView::CopySelectedText( AudaciumProject &project )
{
   if ( !HasSelection( project ) )
      return false;
...
bool LabelTrackView::PasteSelectedText(
   AudaciumProject &project, double sel0, double sel1 )
{
   const auto pTrack = FindLabelTrack();
...
int LabelTrackView::GetSelectedIndex( AudaciumProject &project ) const
{
...
bool LabelTrackView::DoCaptureKey(
   AudaciumProject &project, wxKeyEvent & event )
{
...
unsigned LabelTrackView::CaptureKey(
   wxKeyEvent & event, ViewInfo &, wxWindow *, AudaciumProject *project )
{
...
unsigned LabelTrackView::Char(
   wxKeyEvent & event, ViewInfo &viewInfo, wxWindow *, AudaciumProject *project)
{
   double bkpSel0 = viewInfo.selectedRegion.t0(),
      bkpSel1 = viewInfo.selectedRegion.t1();
   // Pass keystroke to labeltrack's handler and add to history if any
...
bool LabelTrackView::DoKeyDown(
   AudaciumProject &project, NotifyingSelectedRegion &newSel, wxKeyEvent & event)
{
...
bool LabelTrackView::DoChar(
   AudaciumProject &project, NotifyingSelectedRegion &WXUNUSED(newSel),
   wxKeyEvent & event)
{
   // Check for modifiers and only allow shift.
   //
...
void LabelTrackView::ShowContextMenu( AudaciumProject &project )
{
...
void LabelTrackView::OnContextMenu(
   AudaciumProject &project, wxCommandEvent & evt )
{
   auto &selectedRegion = ViewInfo::Get( project ).selectedRegion;
...
bool LabelTrackView::HasSelection( AudaciumProject &project ) const
{
...
(AudaciumProject &project, LabelTrack *lt, int index)
{
   ...
   const SelectedRegion& region, const wxString& initialValue, wxString& value)
{
   ...
   AudaciumTextEntryDialog dialog{ &window,
      XO("Name:"),
      XO("New label"),
      initialValue,
      wxOK | wxCANCEL,
   ...
   // keep the dialog within Audacium's window, so that the dialog is always fully visible
