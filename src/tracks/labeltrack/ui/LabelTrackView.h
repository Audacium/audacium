...
public:
   ...
   bool DoCaptureKey( AudaciumProject &project, wxKeyEvent &event );
   bool DoKeyDown(
      AudaciumProject &project, NotifyingSelectedRegion &sel, wxKeyEvent & event);
   bool DoChar(
      AudaciumProject &project, NotifyingSelectedRegion &sel, wxKeyEvent & event);
...
private:
   ...
   unsigned CaptureKey
     (wxKeyEvent &event, ViewInfo &viewInfo, wxWindow *pParent,
      AudaciumProject *project) override;
...
public:
   static void DoEditLabels(
      AudaciumProject &project, LabelTrack *lt = nullptr, int index = -1);
   ...
   bool IsTextSelected( AudaciumProject &project ) const;
...
public:
   ...
   int GetSelectedIndex( AudaciumProject &project ) const;
   void SetSelectedIndex( int index );

   bool CutSelectedText( AudaciumProject &project );
   bool CopySelectedText( AudaciumProject &project );
...
public:
   ...
   bool CalcCursorX( AudaciumProject &project, int * x ) const;

private:
   void CalcHighlightXs(int *x1, int *x2) const;

public:
   void ShowContextMenu( AudaciumProject &project );

private:
   void OnContextMenu( AudaciumProject &project, wxCommandEvent & evt);
...
public:
   bool HasSelection( AudaciumProject &project ) const;
