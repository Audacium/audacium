...
(const TrackPanelMouseState &st,
 const AudaciumProject *WXUNUSED(project))
{
...
// An example of using an AudaciumCommand simply to create a dialog.
...
class SetTrackNameCommand : public AudaciumCommand
{
public:
...
void TrackMenuTable::OnSetName(wxCommandEvent &)
{
   ...
   if (pTrack)
   {
      AudaciumProject *const proj = &mpData->project;
...
{
   AudaciumProject *const project = &mpData->project;
...
unsigned CommonTrackControls::DoContextMenu(
   const wxRect &rect, wxWindow *pParent, wxPoint *, AudaciumProject *pProject)
{
