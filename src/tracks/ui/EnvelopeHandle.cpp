...
void EnvelopeHandle::Enter(bool, AudaciumProject *)
{
...
namespace {
   void GetTimeTrackData
      (const AudaciumProject &project, const TimeTrack &tt,
...
(std::weak_ptr<EnvelopeHandle> &holder,
 ...
 const AudaciumProject *pProject, const std::shared_ptr<TimeTrack> &tt)
{
...
(std::weak_ptr<EnvelopeHandle> &holder,
 const wxMouseState &state, const wxRect &rect,
 const AudaciumProject *pProject, const std::shared_ptr<WaveTrack> &wt)
{
...
(std::weak_ptr<EnvelopeHandle> &holder,
 const wxMouseState &state, const wxRect &rect, const AudaciumProject *pProject,
 ...
 wxWindow *)
{
   ...
   ProjectHistory::Get( *pProject ).PushState(
      /* i18n-hint: (verb) Audacium has just adjusted the envelope .*/
...
UIHandle::Result EnvelopeHandle::Cancel(AudaciumProject *pProject)
{
