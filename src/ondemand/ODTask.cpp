...
//      wxPrintf("%s %i is %f done\n", GetTaskName(),GetTaskNumber(),PercentComplete());
   ...
   {
      //for profiling, uncomment and look in audacium.app/exe's folder for AudaciumProfile.txt
      //static int tempLog =0;
      //if(++tempLog % 5==0)
         //END_TASK_PROFILING("On Demand Drag and Drop 5 80 mb files into audacium, 5 wavs per task");
...
bool ODTask::IsTaskAssociatedWithProject(AudaciumProject* proj)
{
