...
const wxString HelpSystem::HelpHostname = wxT("alphamanual.audaciumteam.org");
const wxString HelpSystem::HelpServerHomeDir = wxT("/man/");
const wxString HelpSystem::HelpServerManDir = wxT("/man/");
#else
const wxString HelpSystem::HelpHostname = wxT("manual.audaciumteam.org");
...
public:
   ...
   virtual ~HtmlTextHelpDialog()
   {
      ...
      MakeModal( false );
#endif
      // On Windows, for some odd reason, the Audacium window will be sent to
                          ...
                          bool bModal)
{
   ...
   // scripts/mw2html_audacium/mw2html.py
