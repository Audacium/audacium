...
ProjectAudioManager &ProjectAudioManager::Get( AudaciumProject &project )
{
...
ProjectAudioManager::ProjectAudioManager( AudaciumProject &project )
   : mProject{ project }
{
...
WaveTrackArray ProjectAudioManager::ChooseExistingRecordingTracks(
   AudaciumProject &proj, bool selectedOnly, double targetRate)
{
   auto p = &proj;
...
void ProjectAudioManager::OnRecord(bool altAppearance)
{
   ...
   if (p) {
      ...
      if (!allSameRate) {
         AudaciumMessageBox(XO("The tracks selected "
         ...
         if (!existingTracks.empty()) {
            ...
            if (numberOfSelected > 0 && rateOfSelected != options.rate) {
               AudaciumMessageBox(XO(
                  ...
                  "(Audacium requires two channels at the same sample rate for\n"
                  "each stereo track)"),
...
bool ProjectAudioManager::DoRecord(AudaciumProject &project,
...
\n\
Other applications are competing with Audacium for processor time\n\
\n\
...
DefaultPlayOptions( AudaciumProject &project )
{
   auto &projectAudioIO = ProjectAudioIO::Get( project );
...
AudioIOStartStreamOptions
DefaultSpeedPlayOptions( AudaciumProject &project )
{
   auto &projectAudioIO = ProjectAudioIO::Get( project );
   auto gAudioIO = AudioIO::Get();
...
static RegisteredMenuItemEnabler stopIfPaused{{
   ...
   []( AudaciumProject &project, CommandFlag ){
...
GetPropertiesOfSelected(const AudaciumProject &proj)
{
