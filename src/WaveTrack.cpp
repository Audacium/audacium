...
Track::Holder WaveTrack::PasteInto( AudaciumProject &project ) const
{
   auto &trackFactory = WaveTrackFactory::Get( project );
...
//
/*! @excsafety{Weak} -- This WaveTrack remains destructible in case of AudaciumException.
   ...
   for (const auto &clip: mClips)
   {
      ...
      if ((dClipStartTime < endTime) && (dClipEndTime > startTime))
      {
         ...
         if (rt0 + rlen*tstep > dClipEndTime)
         {
            ...
            // This check prevents problem cited in http://bugzilla.audaciumteam.org/show_bug.cgi?id=528#c11,
...
#include "Project.h"
#include "SampleBlock.h"
static auto TrackFactoryFactory = []( AudaciumProject &project ) {
   return std::make_shared< WaveTrackFactory >(
      ProjectSettings::Get( project ),
      SampleBlockFactory::New( project ) );
};

static const AudaciumProject::AttachedObjects::RegisteredFactory key2{
   TrackFactoryFactory
};

WaveTrackFactory &WaveTrackFactory::Get( AudaciumProject &project )
{
   return project.AttachedObjects::Get< WaveTrackFactory >( key2 );
}

const WaveTrackFactory &WaveTrackFactory::Get( const AudaciumProject &project )
{
   return Get( const_cast< AudaciumProject & >( project ) );
}

WaveTrackFactory &WaveTrackFactory::Reset( AudaciumProject &project )
{
...
void WaveTrackFactory::Destroy( AudaciumProject &project )
{
