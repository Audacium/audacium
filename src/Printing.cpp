...
class AudaciumPrintout final : public wxPrintout
{
 public:
   AudaciumPrintout(wxString title,
...
bool AudaciumPrintout::OnPrintPage(int WXUNUSED(page))
{
   wxDC *dc = GetDC();
   if (!dc)
      return false;
...
bool AudaciumPrintout::HasPage(int page)
{
   return (page==1);
}

bool AudaciumPrintout::OnBeginDocument(int startPage, int endPage)
{
   return wxPrintout::OnBeginDocument(startPage, endPage);
}

void AudaciumPrintout::GetPageInfo(int *minPage, int *maxPage,
                                   int *selPageFrom, int *selPageTo)
{
   *minPage = 1;
   *maxPage = 1;
   ...
   TrackPanel &panel)
{
   ...
   AudaciumPrintout printout(name, tracks, panel);
