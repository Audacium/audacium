...
AUDACITY_DLL_API void SetActiveProject(AudaciumProject * project);
...
class AUDACITY_DLL_API AllProjects
{
   ...
   using AProjectHolder = std::shared_ptr< AudaciumProject >;
   using Container = std::vector< AProjectHolder >;
   static Container gAudaciumProjects;
...
public:
   ...
   value_type Remove( AudaciumProject &project );
...
using AttachedProjectObjects = ClientData::Site<
   AudaciumProject, ClientData::Base, ClientData::SkipCopying, std::shared_ptr
>;
...
using AttachedProjectWindows = ClientData::Site<
   AudaciumProject, wxWindow, ClientData::SkipCopying, ClientData::BarePtr
>;
...
///\brief The top-level handle to an Audacium project.  It serves as a source
...
class AUDACITY_DLL_API AudaciumProject final
   : public wxEvtHandler
   , public AttachedProjectObjects
   , public AttachedProjectWindows
   , public std::enable_shared_from_this<AudaciumProject>
{
 public:
...
inline wxFrame *FindProjectFrame( AudaciumProject *project ) {
   return project ? &GetProjectFrame( *project ) : nullptr;
}
inline const wxFrame *FindProjectFrame( const AudaciumProject *project ) {
   return project ? &GetProjectFrame( *project ) : nullptr;
}
