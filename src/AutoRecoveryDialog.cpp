...
{
public:
   explicit AutoRecoveryDialog(AudaciumProject *proj);
...
private:
   ...
   void OnQuitAudacium(wxCommandEvent &evt);
...
BEGIN_EVENT_TABLE(AutoRecoveryDialog, wxDialogWrapper)
   EVT_BUTTON(ID_QUIT_AUDACITY, AutoRecoveryDialog::OnQuitAudacium)
...
AutoRecoveryDialog::AutoRecoveryDialog(AudaciumProject *project)
...
void AutoRecoveryDialog::PopulateOrExchange(ShuttleGui &S)
{
   ...
   {
      S.AddFixedText(
         XO("The following projects were not saved properly the last time Audacium was run and "
...
bool AutoRecoveryDialog::HaveChecked()
{
   ...
   AudaciumMessageBox(XO("No projects selected"), XO("Automatic Crash Recovery"));

   return false;
}

void AutoRecoveryDialog::OnQuitAudacium(wxCommandEvent &WXUNUSED(evt))
{
...
bool ShowAutoRecoveryDialogIfNeeded(AudaciumProject *&pproj, bool *didRecoverAnything)
{
