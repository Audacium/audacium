...
#define AudaciumMixerBoardTitle XO("Audacium Mixer Board%s")
...
MixerTrackCluster::MixerTrackCluster(wxWindow* parent,
                                       MixerBoard* grandParent, AudaciumProject* project,
...
, mTrack{ pTrack }
{
   ...
   // (Still available in Audacium_UmixIt branch off 1.2.6.)
   ...
   if (GetWave()) {
      mMeter =
         safenew MeterPanel(mProject, // AudaciumProject* project,
...
void MixerTrackCluster::UpdateMeter(const double t0, const double t1)
{
      ...
      )
   {
      ...
      // Vaughan, 2011-02-04: Now that we're updating all meters from audaciumAudioCallback,
...
MixerBoardScrolledWindow::MixerBoardScrolledWindow(AudaciumProject* project,
...
MixerBoard::MixerBoard(AudaciumProject* pProject,
                        wxFrame* parent,
                        const wxPoint& pos /*= wxDefaultPosition*/,
                        const wxSize& size /*= wxDefaultSize*/)
: wxWindow(parent, -1, pos, size)
{
   ...
   mScrolledWindow =
      safenew MixerBoardScrolledWindow(
         pProject, // AudaciumProject* project,
...
void MixerBoard::OnTimer(wxCommandEvent &event)
{
   ...
   //v Vaughan, 2011-02-25: Moved this update back here from audaciumAudioCallback.
   //    See note there.
   // Vaughan, 2010-01-30:
   //    Since all we're doing here is updating the meters, I moved it to
   //    audaciumAudioCallback where it calls gAudioIO->mOutputMeter->UpdateDisplay().
...
MixerBoardFrame::MixerBoardFrame(AudaciumProject* parent)
...
{
   AudaciumProject *project = mMixerBoard->mProject;
   auto &commandManager = CommandManager::Get( *project );
   commandManager.FilterKeyEvent(project, event, true);
}

void MixerBoardFrame::Recreate( AudaciumProject *pProject )
{
...
void MixerBoardFrame::SetWindowTitle()
{
   ...
   SetTitle(AudaciumMixerBoardTitle.Format(name).Translation());
}
...
AudaciumProject::AttachedWindows::RegisteredFactory sMixerBoardKey{
