...
/*!********************************************************************
 ...
 @file AudaciumException.h
 @brief Declare abstract class AudaciumException, some often-used subclasses, and @ref GuardedCall
...
{
    Internal, //!< Indicates internal failure from Audacium.
 ...
 */
class AudaciumException /* not final */
{
public:
   AudaciumException() {}
   virtual ~AudaciumException() = 0;
...
protected:
   ...
   AudaciumException( const AudaciumException& ) = default;

   //! Don't allow moves of this class or subclasses
   // see https://bugzilla.audaciumteam.org/show_bug.cgi?id=2442
   AudaciumException( AudaciumException&& ) = delete;

   //! Disallow assignment
   AudaciumException &operator = ( const AudaciumException & ) = delete;
};

//! Abstract AudaciumException subclass displays a message, specified by further subclass
...
class AUDACITY_DLL_API MessageBoxException /* not final */
   : public AudaciumException
{
   //! Privatize the inherited function
   using AudaciumException::DelayedHandlerAction;
...
{
   void operator () (AudaciumException *pException) const
   {
      if ( pException )
...
template <typename R> struct SimpleGuard
{
   explicit SimpleGuard(
      ...
      : m_value{ value } {}
   R operator () ( AudaciumException * ) const { return m_value; }
   const R m_value;
};
...
template<> struct SimpleGuard<bool>
{
   explicit SimpleGuard(
      ...
      : m_value{ value } {}
   bool operator () ( AudaciumException * ) const { return m_value; }
...
{
   SimpleGuard() {}
   void operator () ( AudaciumException * ) const {}
...
/*!
  ...
  If AudaciumException is handled, then it queues up a delayed handler action for execution later in
  the event loop at idle time, on the main thread; typically this informs the user of the error.

  The default delayed handler action is simply to invoke a method of the AudaciumException, but this
  ...
  takes pointer to AudaciumException, which is null when some other type of exception is caught;
  return value is converted to @b R
  @tparam F3 deduced type of delayed handler function, if a nondefault argument is given;
  takes pointer to AudaciumException, return value is unused
 */
template <
   typename R = void,
   ...
   typename F2 = SimpleGuard< R >, // function object
      // with signature R( AudaciumException * )

   typename F3 =
      DefaultDelayedHandlerAction // Any( AudaciumException * ), ignore return
>
//! Execute some code on any thread; catch any AudaciumException; enqueue error report on the main thread
R GuardedCall(
