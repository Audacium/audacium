...
void DoMixAndRender
(AudaciumProject &project, bool toNewTrack)
{
...
void DoPanTracks(AudaciumProject &project, float PanValue)
{
   auto &tracks = TrackList::Get( project );
   auto &window = ProjectWindow::Get( project );
...
void DoAlign
(AudaciumProject &project, int index, bool moveSel)
{
   auto &tracks = TrackList::Get( project );
...
//     audacium-src/win/Release/modules/
#define COLLECT_TIMING_DATA

// Audacium Score Align Progress class -- progress reports come here
...
enum{
   kAudaciumSortByTime = (1 << 1),
   kAudaciumSortByName = (1 << 2),
};

void DoSortTracks( AudaciumProject &project, int flags )
{
   ...
   for (auto iter = tracks.ListOfTracks::begin(),
      ...
      if ( !track->IsLeader() )
         ...
         for (ndx = 0; ndx < size;) {
            ...
            else if(flags & kAudaciumSortByTime) {
...
void SetTrackGain(AudaciumProject &project, WaveTrack * wt, LWSlider * slider)
{
   wxASSERT(wt);
...
void SetTrackPan(AudaciumProject &project, WaveTrack * wt, LWSlider * slider)
{
   wxASSERT(wt);
   float newValue = slider->Get();
...
void OnNewTimeTrack(const CommandContext &context)
{
   ...
   if ( *tracks.Any<TimeTrack>().begin() ) {
      AudaciumMessageBox(
         XO(
"This version of Audacium only allows one time track for each project window.") );
      return;
   }
...
"Alignment error: input too short: MIDI from %.2f to %.2f secs, Audio from %.2f to %.2f secs.")
      ...
      //project.OnUndo(); // recover any changes to note track
      AudaciumMessageBox( XO("Internal error reported by alignment process.") );
   }
}
#endif /* EXPERIMENTAL_SCOREALIGN */
...
{
   auto &project = context.project;
   DoSortTracks(project, kAudaciumSortByTime);
...
{
   auto &project = context.project;
   DoSortTracks(project, kAudaciumSortByName);
