...
bool DoPasteText(AudaciumProject &project)
{
...
bool DoPasteNothingSelected(AudaciumProject &project)
{
   auto &tracks = TrackList::Get( project );
   auto &selectedRegion = ViewInfo::Get( project ).selectedRegion;
...
void OnUndo(const CommandContext &context)
{
   ...
   if (!ProjectHistory::Get( project ).UndoAvailable()) {
      AudaciumMessageBox( XO("Nothing to undo") );
      return;
   }
...
void OnRedo(const CommandContext &context)
{
   ...
   if (!ProjectHistory::Get( project ).RedoAvailable()) {
      AudaciumMessageBox( XO("Nothing to redo") );
      return;
   }
...
// TODO: The comment below suggests this workaround is obsolete.
      ...
      //
      // This workaround should be removed when Audacium updates to wxWidgets
      // 3.x which has a fix.
...
void OnPasteOver(const CommandContext &context)
{
   ...
   if((AudaciumProject::msClipT1 - AudaciumProject::msClipT0) > 0.0)
   {
      selectedRegion.setT1(
         selectedRegion.t0() +
         (AudaciumProject::msClipT1 - AudaciumProject::msClipT0));
      ...
      auto range = TrackList::Get( project ).Any<const LabelTrack>()
         + [&](const LabelTrack *pTrack){
            return LabelTrackView::Get( *pTrack ).IsTextSelected(
               ...
               const_cast<AudaciumProject&>(project)
            );
         };
      if ( !range.empty() )
         return true;
...
BaseItemSharedPtr EditMenu()
{
   ...
   Menu( wxT("Edit"), XXO("&Edit"),
      Section( "UndoRedo",
         ...
         [](AudaciumProject &project, wxMenu&) {
      ...
      // not appear in the Edit menu but instead under Audacium, consistent with
      // MacOS conventions.
      Section( "Preferences",
...
auto canSelectAll = [](const AudaciumProject &project){
   return MenuManager::Get( project ).mWhatIfNoSelection != 0; };
auto selectAll = []( AudaciumProject &project, CommandFlag flagsRqd ){
