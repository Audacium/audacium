...
void DoNextPeakFrequency(AudaciumProject &project, bool up)
{
...
(AudaciumProject &project, double t0)
{
...
double GridMove
(AudaciumProject &project, double t, int minPix)
{
   const auto &settings = ProjectSettings::Get( project );
   auto rate = settings.GetRate();
   auto &viewInfo = ViewInfo::Get( project );
...
(AudaciumProject &project, double seekStep, TimeUnit timeUnit)
{
   auto &viewInfo = ViewInfo::Get( project );
...
(AudaciumProject &project, double direction, SelectionOperation operation,
 SeekInfo &info)
{
...
void DoCursorMove(
   AudaciumProject &project, double seekStep,
...
void DoBoundaryMove(AudaciumProject &project, int step, SeekInfo &info)
{
   auto &viewInfo = ViewInfo::Get( project );
   auto &tracks = TrackList::Get( project );
   auto &window = ProjectWindow::Get( project );
...
static CommandHandlerObject &findCommandHandler(AudaciumProject &project) {
   ...
   Menu( wxT("Select"), XXO("&Select"),
      Section( "Basic",
         ...
         Menu( wxT("Region"), XXO("R&egion"),
            ...
            Section( "",
               // GA: Audacium had 'Store Re&gion' here previously. There is no
               ...
               // Audacium had 'Retrieve Regio&n' here previously.
