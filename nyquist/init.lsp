...
;;
;; Third party plug-ins are not translated by gettext in Audacium, but may include a
...
(defun aud-get-info (str)
  ...
  ;;; Audacium 2.3.0 does not fail if type is not recognised, it 
  ;;; falls back to a default, so test for valid types.
  ;;; 'Commands+' is not supported in Audacium 2.3.0
  (let (type
        info
