...
{
   FilePaths results;
   FindFilesInPathList(code + L"/audacium.mo", pathList, results);
#if defined(__WXMAC__)
   FindFilesInPathList(code + L".lproj/audacium.mo", pathList, results);
#endif
   FindFilesInPathList(code + L"/LC_MESSAGES/audacium.mo", pathList, results);
...
/*         wxLogDebug(wxT("code=%s name=%s fullCode=%s name=%s -> %s"),
   ...
   // JKC: Adding language for simplified audacium.
   {
      wxString code;
      code = wxT("en-simple");
...
wxString SetLang( const FilePaths &pathList, const wxString & lang )
{
   ...
   using future1 = decltype(
      ...
      // xgettext when compiling the catalog template audacium.pot.
