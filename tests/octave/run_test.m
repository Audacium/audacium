#!/usr/bin/octave -qf
## Audacium Octave unit test runner
#
# Max Maisel
#
...
UID=num2str(getuid());
PIPE_TO_PATH=strcat("/tmp/audacium_script_pipe.to.", UID);
PIPE_FROM_PATH=strcat("/tmp/audacium_script_pipe.from.", UID);
TMP_FILENAME=strcat(pwd(), "/tmp.wav");

printf("Open scripting pipes, this may freeze if Audacium does not run...\n");
...
function plot_failure(x, y)
  ...
  hold off
  legend("Audacium", "Octave", "log-delta", "location", "southeast")
