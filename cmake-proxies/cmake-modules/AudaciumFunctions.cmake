...
# Taken from wxWidgets and modified for Audacium
#
...
# the manual. We could add that property to the Audacium target and
# CMake would add the required nodes to the VS project. And when the
# Audacium target is built, the NuGet packages would get automatically
# downloaded. This also means that the locale and manual targets
# must be dependent on the Audacium target so the packages would get
...
#
# We want the Audacium target to be the startup project to provide
# easier debugging. But, if we do that, the ALL_BUILD target is no
# longer "in control" and any dependents of the Audacium target would
...
# not be available during Audacium debugging because the Nyquist runtime
# would not be copied into the destination folder alonside the Audacium
# executable.
#
# To remedy this conundrum, we simply download the NuGet packages
# ourselves and make the Audacium target dependent on the targets
# mentioned above. This ensures that the dest folder is populated
# and laid out like Audacium expects.
#
...
function( audacium_append_common_compiler_options var use_pch )
   if( NOT use_pch )
      list( APPEND ${var}
         PRIVATE
...
function( audacium_module_fn NAME SOURCES IMPORT_TARGETS
...
#   list( TRANSFORM SOURCES PREPEND "${CMAKE_CURRENT_SOURCE_DIR}/" )
   ...
   set( OPTIONS )
   audacium_append_common_compiler_options( OPTIONS NO )
   ...
   if (NOT LIBTYPE STREQUAL "MODULE")
      list( APPEND GRAPH_EDGES "\"Audacium\" -> \"${TARGET}\"" )
   endif ()
...
# may be given too.
macro( audacium_module NAME SOURCES IMPORT_TARGETS
...
# More defines, and more target libraries (maybe generator expressions)
# may be given too.
macro( audacium_library NAME SOURCES IMPORT_TARGETS
   ADDITIONAL_DEFINES ADDITIONAL_LIBRARIES )
...
macro( audacium_header_only_library NAME SOURCES IMPORT_TARGETS
   ADDITIONAL_DEFINES )
   # ditto comment in the previous macro
...
#
#     symbol   suffix for the "USE_<symbol>" variable that the Audacium
...
function( addlib dir name symbol required check )
   ...
   # Let the Audacium target know that this library will be used
