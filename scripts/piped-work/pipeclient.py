...
"""Automate Audacium via mod-script-pipe.

Pipe Client may be used as a command-line script to send commands to
Audacium via the mod-script-pipe interface, or loaded as a module.
...
PipeClient.read : Read Audacium's reply from pipe.
...
if sys.platform == 'win32':
    ...
    # Linux or Mac
    PIPE_BASE = '/tmp/audacium_script_pipe.'
...
class PipeClient():
    """Write / read client access to Audacium via named pipes.
    ...
    ----------
        reader_pipe_broken : event object
            Set if pipe reader fails. Audacium may have crashed
        ...
        reply : string
            message received when Audacium completes the command

    See Also
    --------
    write : Write a command to _write_pipe.
    read : Read Audacium's reply from pipe.
    ...
    def _write_thread_start(self):
        ...
        # freeze if Audacium is not running.
    ...
    def write(self, command, timer=False):
        ...
        ----------
            command : string
                The command to send to Audacium
            timer : bool, optional
    ...
    def read(self):
        """Read Audacium's reply from pipe.

        Returns
        -------
        string
            The reply from the last command sent to Audacium, or null string
            if reply not received. Null string usually indicates that Audacium
