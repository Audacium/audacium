...
# Obtains all menus and all box locations from the currently running Audacium.
# This is useful for wit.audaciumteam.org, where we need these lists when we
...
if( sys.platform  == 'win32' ):
    ...
    toname = '/tmp/audacium_script_pipe.to.' + str(os.getuid())
    fromname = '/tmp/audacium_script_pipe.from.' + str(os.getuid())
    EOL = '\n'

print( "Write to  \"" + toname +"\"" )
if not os.path.exists( toname ) :
   print( " ..does not exist.  Ensure Audacium is running with mod-script-pipe." )
   sys.exit();
