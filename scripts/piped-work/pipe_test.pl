...
# Test script for communicating with audacium via mod-script-pipe
# Audacium should be running first, with the scripting plugin loaded.
#
...
# on the 'Scripting' page of the Audacium wiki.
...
# TODO: Maybe get the pipe names from audacium?
if ($^O eq 'MSWin32') {
   $Name = 'Srv';
   ...
   $UID = $<;
   $ToSrvName = '/tmp/audacium_script_pipe.to.'.$UID;
   $FromSrvName = '/tmp/audacium_script_pipe.from.'.$UID;
} elsif ($^O eq 'darwin') {
   $UID = $<;
   $ToSrvName = '/tmp/audacium_script_pipe.to.'.$UID;
   $FromSrvName = '/tmp/audacium_script_pipe.from.'.$UID;
...
sub testClearAndPasters{
   ...
   # This preferences is not read in Audacium 2.2.x where duration
...
# A list of effects to test (could be got from Audacium in future)
sub getEffects{
...
# The duration setting does not work in Audacium 2.2.x where duration
# is read from pluginsettings.cfg
sub generateBase{
   my $genCmd = "Chirp";
   my $duration = 30.0;
