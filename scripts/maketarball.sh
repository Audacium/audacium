...
function cleanfulltree {
   ...
   myrmvf $1 src/audacium src/.depend src/.gchdepend
...
function slimtree {
   ...
   printf "Removing developer scripts not needed to build audacium ... "
   myrmrvf $1 scripts/mw2html_audacium 
   printf "Done\n"
...
echo "Maketarball 2.1.0 -- make an Audacium distribution tarball"

# check number of arguments, if not one then print a usage message
if [ $# -ne 1 ] ; then
   echo "Script to make directory trees for audacium source tarballs"
   echo "Usage: $0 <mode>"
...
if [ ! -f "src/Audacium.h" ] ; then
   echo "$0 must be run from top-level audacium directory"
   exit 1
fi
...
tmpsrc="${topdir}/$(mktemp -d audacium-src-XXXXXX)" # where initial modifications are done
...
# (because we want the same one that will be used to build Audacium). This is a
...
# you actually need to build audacium on a shared library system with the
...
tarname="audacium-minsrc-${version}" # the directory we will find inside tarballs
mv "${tmpsrc}" "${tarname}"
printf "Done\n"
