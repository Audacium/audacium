...
namespace {
CommandHandlerObject &findme(AudaciumProject&)
{
...
extern "C"
{
   ...
   // ModuleDispatch
   // is called by Audacium to initialize/terminate the module
...
void NyqBench::OnSave(wxCommandEvent & e)
{
   ...
   {
      AudaciumMessageBox(XO("Script was not saved."),
                   XO("Warning"),
                   wxICON_EXCLAMATION,
                   this);
      return;
...
void NyqBench::OnGo(wxCommandEvent & e)
{
   ...
   AudaciumProject *p = GetActiveProject();
   wxASSERT(p != NULL);
...
void NyqBench::OnAbout(wxCommandEvent & e)
{
   ...
   i.SetDescription(_("External Audacium module which provides a simple IDE for writing effects."));
...
void NyqBench::OnFindDialog(wxFindDialogEvent & e)
{
   ...
   if (pos == wxString::npos) {
      AudaciumMessageBox(XO("No matches found"),
...
bool NyqBench::Validate()
{
   if (mScript->GetLastPosition() > 0 && mScript->IsModified()) {
      int ans;
      ans = AudaciumMessageBox(XO("Code has been modified. Are you sure?"),
                         XO("Warning"),
...
//----------------------------------------------------------------------------
// Connects Audacium menu item to an action in this dll.
