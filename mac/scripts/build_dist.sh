...
then
   ...
   then
      ...
      echo "  dstroot   path to where Audacium was built:"
      echo "            legacy build = /tmp/Audacium.dst"
...
# Setup
VERSION=`awk '/^#define+ AUDACITY_VERSION / {print $3}' ${SRCROOT}/../src/Audacium.h`
RELEASE=`awk '/^#define+ AUDACITY_RELEASE / {print $3}' ${SRCROOT}/../src/Audacium.h`
REVISION=`awk '/^#define+ AUDACITY_REVISION / {print $3}' ${SRCROOT}/../src/Audacium.h`
VERSION=$VERSION.$RELEASE.$REVISION
IDENT=$(plist "${DSTROOT}/Audacium.app/Contents/Info.plist" "CFBundleIdentifier")

#
# This depends on a file in the builders HOME directory called ".audacium_signing" that
...
# "org.audaciumteam.audacium" as the application identifier.
#
SIGNING=
if [ -r ~/.audacium_signing ]
then
   source ~/.audacium_signing
...
VOL="Audacium $VERSION"
DMG="audacium-macos-$VERSION"

echo "Audacium has been installed to: ${DSTROOT}"
cd "${DSTROOT}/.."
...
then
   xcrun codesign --verbose \
                  ...
                  --identifier "${IDENT}" \
                  --options runtime \
                  --entitlements "${SRCROOT}/Audacium.entitlements" \
                  --sign "${CODESIGN_APP_IDENTITY}" \
                  ${DSTROOT}/Audacium.app/Contents/modules/*
   ...
   xcrun codesign --verbose \
                  ...
                  --entitlements "${SRCROOT}/Audacium.entitlements" \
                  --sign "${CODESIGN_APP_IDENTITY}" \
                  ${DSTROOT}/Audacium.app/Contents/plug-ins/*
   ...
   xcrun codesign --verbose \
                  ...
                  --entitlements "${SRCROOT}/Audacium.entitlements" \
                  --sign "${CODESIGN_APP_IDENTITY}" \
                  ${DSTROOT}/Audacium.app

   # Create the ZIP archive for notarization
   xcrun ditto -c -k --keepParent "${DSTROOT}/Audacium.app" "${DSTROOT}.zip" 
   ...
   # Staple the app
   stapler staple "${DSTROOT}/Audacium.app"
fi
...
cp "${SRCROOT}/Resources/Audacium-DMG-background.png" "${DMG}/.background"

#Add a custom icon for the DMG
#cp -p mac/Resources/Audacium.icns "${DMG}"/.VolumeIcon.icns
   ...
   tell application "Finder"
     with timeout of 300 seconds
        tell disk "'$TITLE'"
              ...
              set background picture of theViewOptions to file ".background:Audacium-DMG-background.png" 
              make new alias file at container window to POSIX file "/Applications" with properties {name:"Applications"}
              set position of item "Audacium" of container window to {170, 350}
...
rm -rf "${DMG}/Audacium.app/Contents/help"
zip -r9 "${DMG}.zip" "${DMG}"
